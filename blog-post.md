...
> **Update (2026-03-24):** Since the original post, I added three things. First, a **Reactome pathway tool** that fetches biological pathways for each gene target, so the report now shows which pathways a target participates in rather than treating genes in isolation. Second, a **Neo4j graph database** for accumulating results across pipeline runs. Reports are ingested as a knowledge graph (diseases, genes, proteins, compounds, papers, pathways) that enables cross-disease queries like "which targets overlap between Alzheimer's and Parkinson's?" Third, a **web frontend** at [bio.arcosdiaz.com](https://bio.arcosdiaz.com) for interactive exploration of the graph (still work in progress). All three are described below.
...
![Report excerpt: Alzheimer disease targets](figures/report-excerpt.png)
